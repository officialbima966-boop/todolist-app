...
// Cek koneksi database dengan multiple path
$koneksi_paths = [
    __DIR__ . '/../inc/koneksi.php',
    __DIR__ . '/inc/koneksi.php',
    'inc/koneksi.php',
    '../inc/koneksi.php'
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        try {
            ...
            $stmt = $mysqli->prepare("SELECT id, nama, email, password FROM users WHERE username = ?");
            ...
            if (!$user) {
                ...
                if (password_verify($current_password, $user['password'])) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        // OPTIONAL: Update session data jika diperlukan
                        // Anda bisa menambahkan data lain ke session di sini
                        $_SESSION['user_data_updated'] = true;
                        
                        // Reset form fields
                        $_POST = [];
                        
                        // OPSI 1: Tetap di halaman yang sama (rekomendasi)
                        // OPSI 2: Redirect ke halaman login untuk login ulang
                        // header("Location: logout.php?reason=password_changed");
                        // exit;
...
// Ambil data user untuk ditampilkan (setelah proses update)
try {
    $stmt = $mysqli->prepare("SELECT nama, email FROM users WHERE username = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $result = $stmt->get_result();
    $user = $result->fetch_assoc();
