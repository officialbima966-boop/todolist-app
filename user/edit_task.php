...
// Cek session - support untuk user dan admin
...
// Validasi akses: hanya pembuat task yang bisa edit (kecuali admin)
if (!$isAdmin) {
    if ($taskData['created_by'] !== $username) {
        header("Location: tasks.php?error=access_denied");
        exit;
    }
}
...
<body>
                            ...
                            <div class="subtask-item">
                                ...
                                <div class="subtask-assigned">
                                    ...
                                    foreach ($assignedUsers as $assignedUser):
                                        $assignedUser = is_string($assignedUser) ? trim($assignedUser) : trim((string)$assignedUser);
    ...
    <script>
        ...
        // Normalize subtasks data - ensure assigned is always a string
        subtasks = subtasks.map(subtask => ({
            ...subtask,
            assigned: Array.isArray(subtask.assigned) 
                ? subtask.assigned.map(u => String(u).trim()).filter(u => u).join(',') 
                : (typeof subtask.assigned === 'string' ? subtask.assigned : '')
        }));
        ...
        function addUserToSubtask(index) {
            // PERBAIKAN: Tampilkan modal dengan daftar user dari database SAJA
            showUserSelectionModal(index);
        }

        // PERBAIKAN: Fungsi untuk menampilkan modal pemilihan user (HANYA dari database)
        function showUserSelectionModal(subtaskIndex) {
            ...
            // Ambil users yang sudah assigned di subtask ini
            const current = subtasks[subtaskIndex].assigned || '';
            let assignedUsers = [];
            if (Array.isArray(current)) {
                assignedUsers = current.map(u => String(u).trim()).filter(u => u);
            } else if (typeof current === 'string' && current) {
                assignedUsers = current.split(',').map(u => u.trim()).filter(u => u);
            }
            ...
            `;
            header.innerHTML = `
                <h3 style="margin: 0; color: #1f2937; font-size: 20px;">Pilih User untuk Subtask</h3>
            ...
            // Daftar user dari PHP (HANYA dari database)
            ...
            infoText.style.cssText = `
                ...
                color: #0369a1;
            `;
            infoText.innerHTML = '<i class="fas fa-info-circle"></i> Pilih user yang sudah terdaftar di sistem';
            
            // Container untuk user items
            ...
            usersList.forEach(user => {
                ...
                const isAssigned = assignedUsers.includes(userName);
                ...
                userItem.innerHTML = `
                    ...
                    ">${initial}</div>
                    <div style="flex: 1;">
                        <div style="font-weight: 600; color: #1f2937; font-size: 15px;">${escapeHtml(userName)}</div>
                        <div style="font-size: 13px; color: #6b7280; text-transform: capitalize;">${escapeHtml(userRole)}</div>
                    </div>
                    <div style="
                        width: 24px;
                        height: 24px;
                        border: 2px solid ${isAssigned ? '#6366f1' : '#d1d5db'};
                        border-radius: 50%;
                        display: flex;
                        align-items: center;
                        justify-content: center;
                        background: ${isAssigned ? '#6366f1' : 'white'};
                        transition: all 0.2s;
                    ">
                ...
                userItem.onclick = () => {
                    toggleUserInSubtask(subtaskIndex, userName);
        ...
        function toggleUserInSubtask(index, userName) {
            const current = subtasks[index].assigned || '';
            let users = [];
            
            // Handle both array and string formats
            if (Array.isArray(current)) {
                users = current.map(u => String(u).trim()).filter(u => u);
            } else if (typeof current === 'string' && current) {
                users = current.split(',').map(u => u.trim()).filter(u => u);
            }
            ...
            subtasks.forEach((subtask, index) => {
                ...
                if (subtask.assigned) {
                    ...
                    if (Array.isArray(subtask.assigned)) {
                        users = subtask.assigned.map(u => String(u).trim()).filter(u => u);
                    } else if (typeof subtask.assigned === 'string' && subtask.assigned.trim()) {
                        users = subtask.assigned.split(',').map(u => u.trim()).filter(u => u);
                    }
